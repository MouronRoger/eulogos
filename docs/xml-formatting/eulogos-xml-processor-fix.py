"""Enhanced XML processor for Eulogos to properly handle TEI XML formatting.

This module provides a comprehensive upgrade to the existing XMLProcessorService
with improved handling of TEI XML elements, including chapters, footnotes, 
line breaks, page breaks, and proper token-level handling of Greek text.

Simply replace the contents of app/services/xml_processor.py with this file
or apply the key methods to your existing implementation.

Features:
- Detailed handling of TEI XML elements (div, p, note, lb, pb, etc.)
- Proper token-level processing for Greek text
- Support for critical apparatus entries
- Reference-based navigation
- Clean HTML generation that matches CSS selectors
"""
...
class XMLProcessorService:
    """Service class for handling XML file operations with enhanced TEI handling."""

    def __init__(self, data_path: str = "data", catalog_service = None) -> None:
        """Initialize the XML processor service.
        
        Args:
            data_path: Directory containing XML files, defaults to 'data'
            catalog_service: Optional CatalogService for path resolution
        """
    ...
    def tokenize_text(self, element: Element) -> List[Dict[str, Any]]:
        ...
        # Use regex that properly handles Greek Unicode characters
        words = re.findall(r'[\p{L}\p{M}]+|[^\p{L}\p{M}\s]|\s+', text, re.UNICODE)

        for word in words:
            if word.strip():
                token_type = "word" if re.match(r'[\p{L}\p{M}]+', word, re.UNICODE) else "punct"
                tokens.append({"type": token_type, "text": word, "index": word_index})
                if token_type == "word":
                    word_index += 1
    ...
    def _process_element_to_html(self, element: Element, parent_ref: str = "") -> str:
        ...
        if tag == "div" and n_value:
            ...
            # Line break with line number
            line_num = f' data-n="{n_value}"' if n_value else ''
            html = f'<br class="lb"{line_num}>'
        elif tag == "pb":
            # Page break with page number
            page_num = n_value or ""
            html = f'<div class="pb" data-n="{page_num}">Page {page_num}</div>'
        elif tag == "note":
            note_type = f' {type_attr}' if type_attr else ''
            html = f'<div class="note{note_type}">'
        elif tag == "quote":
        ...
        if element.text and element.text.strip():
            if tag in ["ref", "foreign", "bibl"]:
                ...
                for token in tokens:
                    if token.strip():
                        if re.match(r'^[\p{L}\p{M}]+$', token, re.UNICODE):  # Word token (handles Greek)
        ...
        for child in element:
            ...
            if child.tail and child.tail.strip():
                tokens = re.findall(r'([\p{L}\p{M}]+|[^\p{L}\p{M}\s]|\s+)', child.tail, re.UNICODE)
                for token in tokens:
                    if token.strip():
                        if re.match(r'^[\p{L}\p{M}]+$', token, re.UNICODE):
                            html += f'<span class="token" data-token="{token}">{token}</span>'
                        else:
                            html += f'<span class="punct">{token}</span>'
                    else:
